...
_PENDING_APPROVAL_STATUSES = {"pending", "queued", ""}
...
async def run_step_activity(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    _check(resp, op=f"run_step[{step_type}]")
...
async def wait_for_approval_activity(payload: dict[str, Any]) -> dict[str, Any]:
    """Poll `GET /approvals/{id}` until decided, expired, or the deadline passes.

    Returns `{"approval_id", "status", "approved": bool, "note"}`. A timeout is a
    normal (non-throwing) outcome so the workflow can abort cleanly.
    """
    settings = get_settings()
    approval_id = str(payload["approval_id"])
    timeout_s = float(payload.get("timeout_seconds") or settings.worker_approval_timeout_seconds)
    ...
    async with _client(settings) as client:
        while True:
            ...
            if status not in _PENDING_APPROVAL_STATUSES:
            ...
            if time.monotonic() >= deadline:
                _warn("wait_approval.timeout", approval_id=approval_id, polls=polls)
                return {
                    "approval_id": approval_id,
                    "status": "timed_out",
                    "approved": False,
